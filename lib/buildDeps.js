...
module.exports = function buildDeps(context, mainModule, options, callback) {
    ...
    addModule(depTree, context, mainModule, options, (err, moduleId) => {
        addChunk(depTree, depTree.modulesById[moduleId], options);
                ...
                requiresKeys.forEach(requiredModuleName => {
                    addModule(depTree, path.dirname(filename),requiredModuleName, options, (err, moduleId) => {
                        ...
                        if (err) {
                            ...
                            requires[requiredModuleName].forEach(requiredModule => {
                                requiredModule.id = moduleId;
                            })
                        }
...
function packModule(source) {
    ...
    currentModule.asyncs = deps.asyncs || [];

    return {currentModule, requires: parseRequires(currentModule)};
}
	...
	if (context.chunks.indexOf(chunkId) === -1) {
        ...
        if (context.requires) {
            context.requires.forEach(requireModule => {
                if (requireModule.id) {
                    addModuleToChunk(depTree, depTree.modulesById[requireModule.id], chunkId, options);
                }
            });
        }

        if (context.asyncs) {
            context.asyncs.forEach(requireModule => {
                addChunk(depTree, requireModule, options);
            });
        }
    }
}
